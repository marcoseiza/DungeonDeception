#ifndef CONTROLLERS_ACTIONS_SETTINGS_H_
#define CONTROLLERS_ACTIONS_SETTINGS_H_
#include <cugl/cugl.h>
...
class Settings : public Action {
 ...
 public:
  ...
  /**
   * This method allocates Settings and initializes it.
   *
   * @param assets The loaded assets for this game mode.
   * @param bounds The scene2 game bounds.
   * @return A newly allocated Settings action.
   */
  static std::shared_ptr<Settings> alloc(
      const std::shared_ptr<cugl::AssetManager> &assets, cugl::Rect bounds) {
    std::shared_ptr<Settings> result = std::make_shared<Settings>();
   ...
   */
  bool didOpenSettings() const { return _prev_down && !_curr_down; }
...
#endif /* CONTROLLERS_ACTIONS_SETTINGS_H_ */
