...
bool DepositEnergyScene::init(
  ...
  _energy_bar = std::dynamic_pointer_cast<cugl::scene2::ProgressBar>(
      _node->getChildByName("energy-bar")->getChildByName("bar"));
  _corrupted_energy_bar = std::dynamic_pointer_cast<cugl::scene2::ProgressBar>(
      _node->getChildByName("corrupted-energy-bar")->getChildByName("bar"));
...
void DepositEnergyScene::start(int terminal_room_id) {
  ...
  _done_butt = std::dynamic_pointer_cast<cugl::scene2::Button>(
      _node->getChildByName("done-button-wrapper")->getChildByName("button"));
