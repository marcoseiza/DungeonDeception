...
//
//  This module provides support for an abstracted networking layer on top of
//  Slikenet. It utilizes a simple ad hoc lobby system inspired by Family Style
//  and Sweet Space. If you use this class, you should create your own lobby
//  using the provided docker container here:
//
...
/** The maximum byte vector size.  1430 (VPN Max) - 40 (IP HEADER) - 6 (CUGL
 * HEADER) */
...
NetworkConnection::NetworkConnection()
    : _debug(true), _apiVer(0), _numPlayers(1), _maxPlayers(1) {
 ...
 * Wait for {@link #getStatus} to return CONNECTED. Once it does, {@link
 * #getPlayerID} will return your assigned player ID.
 *
 * @param setup     Connection config
  ...
  switch (reliability) {
    ...
    case UNRELIABLE_SEQUENCED:
      CUAssertLog(
          ...
          "Unreliable messages are only permitted after initial connection.");
      _reliability = reliability;
      break;
    default:
...
void writeBs(SLNet::BitStream& bts, uint8_t head,
  ...
  CUAssertLog(length < MAX_PACKET,
              "Message size %d exceeds packet capacity (%d)", length,
              MAX_PACKET);
  if (length < 255) {
...
void NetworkConnection::c0StartupConn() {
  _peer = std::unique_ptr<SLNet::RakPeerInterface>(
  ...
  if (_debug) {
    CULog("Your GUID is: %s",
          _peer->GetGuidFromSystemAddress(SLNet::UNASSIGNED_SYSTEM_ADDRESS)
              .ToString());
  }
 ...
 */
void cugl::NetworkConnection::ch2HostGetRoomID(HostPeers& h,
                                               SLNet::BitStream& bts) {
 ...
 */
void NetworkConnection::cc2ClientPunchSuccess(ClientPeer& c,
                                              SLNet::Packet* packet) {
 ...
 */
void NetworkConnection::cc3HostReceivedPunch(HostPeers& h,
                                             SLNet::Packet* packet) {
 ...
 */
void NetworkConnection::cc4ClientReceiveHostConnection(ClientPeer& c,
                                                       SLNet::Packet* packet) {
 ...
 */
void NetworkConnection::cc5HostConfirmClient(HostPeers& h,
                                             SLNet::Packet* packet) {
 ...
 */
void NetworkConnection::cc6ClientAssignedID(
    ClientPeer& c, const std::vector<uint8_t>& msgConverted) {
  bool apiMatch = msgConverted[3] == _apiVer;
  if (!apiMatch) {
    if (_debug) {
      CULogError("API version mismatch; currently %d but host was %d", _apiVer,
                 msgConverted[3]);
    }
 ...
 */
void NetworkConnection::cc7HostGetClientData(
    HostPeers& h, SLNet::Packet* packet,
  ...
  for (uint8_t i = 0; i < h.peers.size(); i++) {
    if (*h.peers.at(i) == packet->systemAddress) {
      ...
      if (msgConverted[1] == 0) {
        if (_debug) {
          CULog("Client %d reported outdated API or other issue; disconnecting",
                pID);
        }
        _peer->CloseConnection(packet->systemAddress, true);
        return;
      ...
      if (_debug) {
        CULog(
            "Player id %d was successfully verified; connection handshake "
            "complete",
            pID);
      }
 ...
 */
void cugl::NetworkConnection::cr1ClientReceivedInfo(
    ClientPeer& c, const std::vector<uint8_t>& msgConverted) {
  if (_debug) {
  ...
  if (!success) {
    ...
    if (_debug) {
      CULogError(
          "Invalid reconnection target; we are player ID %d but host thought "
          "we were %d",
  ...
  directSend({static_cast<uint8_t>(_playerID.has_value() ? *_playerID : 0),
 ...
 */
void NetworkConnection::cr2HostGetClientResp(
    HostPeers& h, SLNet::Packet* packet,
    const std::vector<uint8_t>& msgConverted) {
  if (_debug) {
...
void NetworkConnection::send(const std::vector<uint8_t>& msg,
  ...
  std::visit(make_visitor(
                 [&](HostPeers& /*h*/) {
                   _peer->Send(&bs, MEDIUM_PRIORITY, _reliability, 1,
                               *_natPunchServerAddress, true);
                 },
                 [&](ClientPeer& c) {
                   if (c.addr == nullptr) {
                     return;
                   }
                   _peer->Send(&bs, MEDIUM_PRIORITY, _reliability, 1, *c.addr,
                               false);
                 }),
             _remotePeer);
}
 ...
 *
 * This is only useful when called from a client (player ID != 0). As host, this
 * is method does nothing. Within a few frames, the host should receive this via
 * a call to {@link #receive}
 *
 * This requires a connection be established. Otherwise its behavior is
 * undefined.
 *
...
void NetworkConnection::sendOnlyToHost(const std::vector<uint8_t>& msg) {
  std::visit(make_visitor([&](HostPeers& /*h*/) {},
                          [&](ClientPeer& c) { send(msg, DirectToHost); }),
             _remotePeer);
}
 ...
 *
 * This method must be called periodically EVEN BEFORE A CONNECTION IS
 * ESTABLISHED. Otherwise, the library has no way to receive and process
 ...
 * during the network connection phase, before the game starts, this method
 * should be called every frame. Otherwise, the NAT Punchthrough library may
 * fail. Afterwards, you can delay this to every few frames if necessary to
 * relieve congestion.
 *
    ...
    switch (packet->data[0]) {
      case ID_CONNECTION_REQUEST_ACCEPTED:
        ...
        if (packet->systemAddress == *(_natPunchServerAddress)) {
          // Punchthrough server
          std::visit(
              make_visitor([&](HostPeers& h) { ch1HostConnServer(h); },
      ...
      case ID_NEW_INCOMING_CONNECTION:  // Someone connected to you
        ...
        std::visit(make_visitor(
                       [&](HostPeers& /*h*/) {
                         CULogError("How did that happen? You're the host");
                       },
                       [&](ClientPeer& c) {
            ...
            make_visitor(
                [&](HostPeers& h) {
                  for (uint8_t i = 0; i < h.peers.size(); i++) {
                    ...
                    if (*h.peers.at(i) == packet->systemAddress) {
                      ...
                      if (_peer->GetConnectionState(packet->systemAddress) ==
                          SLNet::IS_CONNECTED) {
                        _peer->CloseConnection(packet->systemAddress, true);
                      }
                      return;
                ...
                [&](ClientPeer& c) {
                  if (packet->systemAddress == *_natPunchServerAddress) {
                    if (_debug) {
                      CULog(
                          "Successfully disconnected from Punchthrough server");
                    }
                  }
                  if (packet->systemAddress == *c.addr) {
                    if (_debug) {
      ...
      case ID_NAT_TARGET_UNRESPONSIVE: {
        ...
        CULogError("Attempted punchthrough to GUID %s failed",
      ...
      case ID_USER_PACKET_ENUM + Standard: {
        ...
        std::visit(make_visitor(
                       [&](HostPeers& /*h*/) {
                         broadcast(msgConverted, packet->systemAddress);
                       },
                       [&](ClientPeer& c) {}),
                   _remotePeer);
      ...
      case ID_USER_PACKET_ENUM + DirectToHost: {
        ...
        std::visit(
            make_visitor([&](HostPeers& /*h*/) { dispatcher(msgConverted); },
                         [&](ClientPeer& c) {
                           CULogError(
                               "Received direct to host message as client");
                         }),
            _remotePeer);
      ...
      case ID_USER_PACKET_ENUM + AssignedRoom: {
        std::visit(make_visitor([&](HostPeers& h) { ch2HostGetRoomID(h, bts); },
                                [&](ClientPeer& c) {
                                  if (_debug) {
      ...
      case ID_USER_PACKET_ENUM + Reconnect: {
        ...
        std::visit(
            make_visitor(
                [&](HostPeers& h) {
                  cr2HostGetClientResp(h, packet, msgConverted);
                },
      ...
      case ID_USER_PACKET_ENUM + PlayerJoined: {
        ...
        std::visit(make_visitor(
                       [&](HostPeers& h) {
                         CULogError("Received player joined message as host");
                       },
                       [&](ClientPeer& c) {
      ...
      case ID_USER_PACKET_ENUM + PlayerLeft: {
        ...
        std::visit(make_visitor(
                       [&](HostPeers& h) {
                         CULogError("Received player left message as host");
                       },
                       [&](ClientPeer& c) {
...
void NetworkConnection::startGame() {
  ...
  std::visit(make_visitor(
                 [&](HostPeers& h) {
                   h.started = true;
                   broadcast({},
                             const_cast<SLNet::SystemAddress&>(
