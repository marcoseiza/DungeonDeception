...
class Obstacle {
 protected:
  ...
  std::vector<std::shared_ptr<b2FixtureDef>> _sensor_defs;
...
#pragma mark BodyDef Methods
   ...
   */
  virtual std::vector<std::shared_ptr<b2FixtureDef>> getSensors() {
    return _sensor_defs;
  }
  ...
  virtual void addSensors(
      std::vector<std::shared_ptr<b2FixtureDef>> sensor_defs) {
    _sensor_defs = sensor_defs;
  }
...
#pragma mark Garbage Collection
   ...
   *
   * Attributes tied to the geometry (and not just forces/position) must wait
   * for collisions to complete before they are reset.  Shapes (and their
